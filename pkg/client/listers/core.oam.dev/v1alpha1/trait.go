/*

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/
// Code generated by lister-gen. DO NOT EDIT.

package v1alpha1

import (
	v1alpha1 "github.com/oam-dev/oam-go-sdk/apis/core.oam.dev/v1alpha1"
	"k8s.io/apimachinery/pkg/api/errors"
	"k8s.io/apimachinery/pkg/labels"
	"k8s.io/client-go/tools/cache"
)

// TraitLister helps list Traits.
type TraitLister interface {
	// List lists all Traits in the indexer.
	List(selector labels.Selector) (ret []*v1alpha1.Trait, err error)
	// Traits returns an object that can list and get Traits.
	Traits(namespace string) TraitNamespaceLister
	TraitListerExpansion
}

// traitLister implements the TraitLister interface.
type traitLister struct {
	indexer cache.Indexer
}

// NewTraitLister returns a new TraitLister.
func NewTraitLister(indexer cache.Indexer) TraitLister {
	return &traitLister{indexer: indexer}
}

// List lists all Traits in the indexer.
func (s *traitLister) List(selector labels.Selector) (ret []*v1alpha1.Trait, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Trait))
	})
	return ret, err
}

// Traits returns an object that can list and get Traits.
func (s *traitLister) Traits(namespace string) TraitNamespaceLister {
	return traitNamespaceLister{indexer: s.indexer, namespace: namespace}
}

// TraitNamespaceLister helps list and get Traits.
type TraitNamespaceLister interface {
	// List lists all Traits in the indexer for a given namespace.
	List(selector labels.Selector) (ret []*v1alpha1.Trait, err error)
	// Get retrieves the Trait from the indexer for a given namespace and name.
	Get(name string) (*v1alpha1.Trait, error)
	TraitNamespaceListerExpansion
}

// traitNamespaceLister implements the TraitNamespaceLister
// interface.
type traitNamespaceLister struct {
	indexer   cache.Indexer
	namespace string
}

// List lists all Traits in the indexer for a given namespace.
func (s traitNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Trait, err error) {
	err = cache.ListAllByNamespace(s.indexer, s.namespace, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Trait))
	})
	return ret, err
}

// Get retrieves the Trait from the indexer for a given namespace and name.
func (s traitNamespaceLister) Get(name string) (*v1alpha1.Trait, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("trait"), name)
	}
	return obj.(*v1alpha1.Trait), nil
}
