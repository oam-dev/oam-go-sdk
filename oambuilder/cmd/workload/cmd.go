...
var (
	logger = ctrl.Log.WithName("oambuilder.workload")
)

var Workload = &cobra.Command{
	Use:                "workload",
	Short:              "generate executor for workload operator.",
	Long:               "generate executor for workload operator.",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		g.AttachTemplate(templates.WorkloadControllerTemplate(), types.TemplateType_Controller)
		...
		if err := ctx.UpdateProject(g, types.ResourceType_Workload); err != nil {
...
import (
	...
	types "github.com/oam-dev/oam-go-sdk/oambuilder/pkg/types/workload"
...
// {{.Resource.Kind}}Settings defines the state described in workloadtype.core.oam.dev
type {{.Resource.Kind}}Settings struct {
	// INSERT ADDITIONAL SETTINGS FIELDS - desired state described in workloadtype.core.oam.dev
...
type {{.Resource.Kind}}Spec struct {
	...
	Component *oamv1alpha1.ComponentSchematic ` + "`" + `json:"component"` + "`" + `
	Traits []types.Trait ` + "`" + `json:"traits,omitempty"` + "`" + `
